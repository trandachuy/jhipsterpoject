package com.trandachuy.myapp.web.rest;
...
import com.trandachuy.myapp.IntegrationTest;
import com.trandachuy.myapp.config.EmbeddedKafka;
import com.trandachuy.myapp.config.KafkaSseConsumer;
import com.trandachuy.myapp.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class JhipsterProjectApplicationKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jhipster-project-application-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jhipster-project-application-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/jhipster-project-application-kafka/unregister"));
                return;
            }
        }
